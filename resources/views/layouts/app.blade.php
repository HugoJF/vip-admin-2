...
<head>
    ...
    <title>@yield('title', 'Dashboard') | VIP Servidores de_nerdTV</title>

    <!-- Fonts -->
    <link href="https://fonts.googleapis.com/css?family=Cairo&subset=latin" rel="stylesheet">
    ...
    <main class="flex flex-col md:flex-row md:flex-wrap">
        <nav class="w-full block md:w-1/3 lg:w-1/5 md:fixed light sidebar bg-gray-900">
            <div class="sidebar-sticky p-4">
                ...
                @auth
                    @if(($warnings = auth()->user()->warnings()) && !empty($warnings))
                        ...
                        <ul class="flex flex-col items-center">
                            @foreach ($warnings as $warning)
                                <li><a href="{{ $warning['url'] }}" class="badge badge-{{ $warning['level'] }}">{{ $warning['message'] }}</a></li>
                            @endforeach
                        </ul>
                    @endif
                @endauth
